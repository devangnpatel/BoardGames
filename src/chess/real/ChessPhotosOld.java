...
public class ChessPhotosOld {
    ...
    public HashMap<String,BufferedImage> pieceImages = null;
    ...
    /**
     * Gets the image of a chess piece (of type and color) at the input row,col pair
     * - If the photo is not already loaded, the appropriate photo file is loaded
     *   and stored in a HashMap for all future use
     * @param pieceType King,Queen,Bishop,Rook,Knight,Pawn
     * @param pieceColor White or Black
     * @param x Column on board of requested piece
     * @param y Row on board of requested piece
     * @return Image of the requested chess piece at the given location
     */
    public BufferedImage getPieceImage(String pieceType,String pieceColor,int x,int y)
    {
        String filenameHash = getPieceFilename(pieceType,pieceColor,x,y);
        if (!pieceImages.containsKey(filenameHash))
        {
            BufferedImage tmpImage = loadPieceImage(pieceType,pieceColor,x,y);
            if (tmpImage != null)
            {
